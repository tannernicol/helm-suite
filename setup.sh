...
dirs=(
    ...
    "/srv/helmv2/caddy"
    "/srv/helmv2/authelia"
    "/srv/helmv2/gitea/data"
    "/srv/helmv2/gitea/config"
    "/srv/helmv2/immich/upload"
    "/srv/helmv2/immich/postgres"
    "/srv/helmv2/grafana/data"
    "/srv/helmv2/searxng/config"
    "/srv/helmv2/ollama/models"
    "/srv/helmv2/monitoring"
)
...
CADDY_OUT="/srv/helmv2/caddy/Caddyfile"
...
for site_template in "${SCRIPT_DIR}"/infra/caddy/sites/*.caddy; do
    ...
    envsubst < "$site_template" > "/srv/helmv2/caddy/sites_${site_name}"
    ok "Generated site: ${site_name}"
done

# Copy snippets
cp "${SCRIPT_DIR}"/infra/caddy/snippets/*.caddy /srv/helmv2/caddy/ 2>/dev/null || true
ok "Copied Caddy snippets"

# -- Generate Authelia Config -------------------------------------------------
header "Generating Authelia Configuration"

AUTHELIA_OUT="/srv/helmv2/authelia/configuration.yml"
...
Type=exec
User=${USERNAME}
ExecStart=/usr/bin/caddy run --config /srv/helmv2/caddy/Caddyfile
ExecReload=/usr/bin/caddy reload --config /srv/helmv2/caddy/Caddyfile
Restart=always
RestartSec=10
TimeoutStartSec=30
TimeoutStopSec=30
...
[Unit]
Description=Run HelmV2 backup ${BACKUP_SCHEDULE:-daily}
...
${BOLD}Generated files:${NC}
  ${DIM}Caddyfile:${NC}      /srv/helmv2/caddy/Caddyfile
  ${DIM}Authelia:${NC}       /srv/helmv2/authelia/configuration.yml
  ${DIM}Shared .env:${NC}    /srv/helmv2/.env
